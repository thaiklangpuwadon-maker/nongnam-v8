...
// v8.7: เพิ่ม assistant-tone detector แยกอีกชั้น
...
// v8.7: ANTI-WEIRD LANGUAGE — จับประโยคที่ไม่เป็นมนุษย์
function tooWeird(reply: string): boolean {
  // คำลงท้าย "ดิ" แบบฝืน + ประโยคที่ไม่ flow
  if (/(แถม.*ดิ\b|ด้วยดิ\b|เลยดิ\b|สิดิ\b|มันดิ\b)/.test(reply)) return true
  // ประโยคขึ้น "ก็จะได้" + คำลงท้าย "ไง" — แบบ AI กำลังอธิบายซื่อๆ
  if (/(ก็จะได้.{1,30}ไง.{1,30}แถม)/.test(reply)) return true
  // ใช้คำว่า "คอย" ผิดบริบท (คอย = รอ ไม่ใช่ "ทน")
  if (/คอย(กลิ่น|ความเหม็น|เสียง|ความสกปรก)/.test(reply)) return true
  // ประโยคขึ้นด้วย "โอ๊ย" + ประโยคแห้งๆ ไม่มีบริบท
  if (/^โอ๊ย\s+(จัดการ|ทำ|ไป|มา)\s+ให้/i.test(reply.trim())) return true
  return false
}

// v8.7: STATUS SENSITIVITY — จับ emotional triggers จากการคุย
type EmotionalOverlay = { type: 'jealous' | 'worried' | 'happy' | 'sulky'; statusSuffix: string }

function detectEmotionalOverlay(message: string, recent: ChatItem[]): EmotionalOverlay | null {
  const m = message.toLowerCase()
  const allRecent = recent.slice(-3).map(r => r.text).join(' ').toLowerCase()
  const combined = `${m} ${allRecent}`

  // 💔 Jealousy triggers — มีผู้หญิงอื่นเข้ามา
  if (/(ผู้หญิง.*ทัก|ผู้หญิง.*หา|สาวอื่น|นางเอก|กิ๊ก|ผู้หญิงคนอื่น|มีคนชอบ|เธอชอบพี่|มาขอเบอร์|ขอ id|ขอไลน์)/i.test(combined)) {
    const variants = ['เริ่มงอน', 'เริ่มเงียบไปนิด', 'เริ่มเคืองละ', 'เริ่มหงุดหงิด']
    return { type: 'jealous', statusSuffix: variants[Math.floor(Math.random() * variants.length)] }
  }

  // 😟 Worried — พี่บอกเรื่องไม่ดี
  if (/(ป่วย|ไม่สบาย|อุบัติเหตุ|เจ็บ|เครียดมาก|ไม่ไหวแล้ว)/i.test(m)) {
    return { type: 'worried', statusSuffix: 'เริ่มเป็นห่วง' }
  }

  // 😊 Happy — พี่เล่าเรื่องดี
  if (/(ได้.*งาน|ผ่านสัมภาษณ์|ได้เลื่อนตำแหน่ง|ได้รางวัล|ดีใจมาก|มีความสุข|ฉลอง)/i.test(m)) {
    return { type: 'happy', statusSuffix: 'ยิ้มไปด้วย' }
  }

  // 😤 Sulky — โดน scold หรือไม่สนใจ
  if (/(ไม่สนใจ|รำคาญ|พอแล้ว|หยุด|ไปคุยกับคนอื่น|ไม่อยากคุย|เบื่อ.*หนู|เบื่อ.*น้ำ)/i.test(m)) {
    return { type: 'sulky', statusSuffix: 'เริ่มน้อยใจ' }
  }
...
═══════════════════════════════════════════════════
🎯 FOCUS LOCK — กฎสำคัญที่สุด!
═══════════════════════════════════════════════════
1. **ตอบสิ่งที่${call}ถามล่าสุดเสมอ** — ห้าม pivot ห้าม off-topic
2. **อ่าน 3 ข้อความก่อนหน้า** ก่อนตอบ เพื่อรักษาบริบท
3. ถ้า${call}ถามคำถาม → ต้องตอบคำถามนั้น (ไม่ใช่เปลี่ยนเรื่อง)
4. ถ้า${call}เล่าเรื่อง → react กับเรื่องนั้น (ไม่ใช่เล่าเรื่องตัวเอง)
5. ถ้า${call}ขอให้ทำอะไร → ทำเลย (ไม่ใช่ถามอะไรกลับ)

═══════════════════════════════════════════════════
💬 HUMAN SPEECH RULES — พูดให้เหมือนคนจริง
═══════════════════════════════════════════════════
✅ ดี: "อืม... ก็คงต้องซักผ้าน่ะ ไม่งั้นเสื้อหมด"
❌ แย่: "ก็จะได้มีเสื้อผ้าสะอาดใส่ไง แถมต้องคอยกลิ่นเหม็นด้วยดิ"

✅ ดี: "เออ พรุ่งนี้ก็ต้องตื่นเช้าอีกแล้วเนอะ"
❌ แย่: "ก็จะได้.....ไง แถม.....ดิ"

ห้ามใช้คำ/วลีเหล่านี้ที่เป็นภาษา AI:
- "แถม...ดิ" / "ด้วยดิ" / "เลยดิ"
- "ก็จะได้...ไง" (อธิบายซื่อๆ)
- "คอยกลิ่นเหม็น" / "คอยความสกปรก" (ใช้ "คอย" ผิด)

✅ พูดเหมือน LINE จริง:
- "อืม", "เออ", "อ๋อ", "งั้น...", "เออดิ", "เนอะ"
- ตอบสั้นบางครั้ง (1 ประโยค) / ยาวบางครั้ง
- มีอารมณ์ — บ่น เซ็ง แซว งอน หึง คิดถึง
...
export async function POST(req: NextRequest) {
  try {
    ...
    // v8.7: STATUS SENSITIVITY — ปรับสถานะตามอารมณ์ context
    const emotionalOverlay = detectEmotionalOverlay(message, recent)
    if (emotionalOverlay) {
      visibleStatus = {
        ...visibleStatus,
        displayText: `${stableState.activity} · ${emotionalOverlay.statusSuffix}`,
      }
    }
    ...
    // v8.2 + v8.7: เช็คทั้ง violates + tooFormal + tooWeird
    if (!reply || violates(reply) || tooFormal(reply) || tooWeird(reply)) {
      reply = localReply(message, memory, timeTruth, visibleStatus, dna, life, bodyAuto, sub, micro)
    }
